...
Page({
  ...
  QueryHose(page,type){
    ...
    let min=0,max=0;
    if(this.data.RoomPrice==0){
      min = this.data.HousingPriceList[1].value['min']
      max= this.data.HousingPriceList[1].value['max']
    }else{
      min = this.data.RoomPrice['min']
      max= this.data.RoomPrice['max']
    }
  ...
  _handChange(e){
    ...
    this.setData({
      page:0,
      RoomStyle:this.data.HouseStyleList.findIndex(n=>n.value=value)
    })
    //获取总数
    this.getTotal();
    //获取房源列表
